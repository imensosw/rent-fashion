...
-->
	   ...
	   <div class="column main-content" id="content_container">
		...
		<?php
			...
			$sql = mysql_query("SELECT * FROM {$statement} WHERE author='" . mysql_real_escape_string($_SESSION['ps_usern']) . "' ORDER BY id DESC LIMIT {$startpoint} , {$limit}");
		  ...
		  <div class="six wide column">
			<div class="ui list">
			  <div class="item">
				...
				<div class="content">
				  <div class="header"><?php echo protect($row['id']); ?></div>
				  —
			...
			<a class="ui basic button product-edit-old" title="Edit this product" href="<?php echo $web['url']; ?>edit/<?php echo protect($row['id']); ?>" data-pushstate="true">
			  Edit
			</a>
		  </div>
		  <div class="four wide column">
			  <center>
				<?php if($row['price_extended'] == 0.00) { ?>
				  <?php if($row['downloads'] == 0) { echo '0 downloads'; } else { echo intval($row['downloads'])." downloads"; } ?>
				<?php } ?>

				<?php if($row['price_extended'] > 0.00) { ?>
				  <?php if($row['sales'] == 1) { echo '1 sale'; } else { echo intval($row['sales']) ." sales"; } ?>
				<?php } ?>
			  </center>
		  </div>
