...
<html lang="en">
	<head>
		...
		<?php include($_SERVER['DOCUMENT_ROOT']."/hautehire/static/gen/modal/header-modal.php"); ?>
...
<body class="landing-page">
  ...
  <div class="sellfy-landing-header">
    <div class="ui grid global-wrap">
	  ...
	  <?php if(!$_SESSION['ps_usern']) { ?>
          ...
          <a class="ui tiny flat green button hide-mobile" id="modal_trigger" href="<?php echo $web['url']; ?>account/register">Sign up</a>
          <a class="ui tiny flat green button hide-desktop" href="<?php echo $web['url']; ?>account/login">Log in</a>  
          <select class="ui tiny flat green button hide-mobile" placeholder="Manu">
               <option value="">How it Works</option>
               <option value="">Rent</option>
               <option value="">Lend</option>
               <option value="">About</option>
               <option value="">us Help & faq</option>
          </select>
          <a class="ui tiny flat green button hide-mobile" id="modal_trigger" href="<?php echo $web['url']; ?>account/register">Post a new listing</a>
	  <?php } ?>
  ...
  <div class="landing-header">
    <div class="ui global-wrap">
	  <h1 class="ui huge normal header inverted" style="text-transform:uppercase;font-weight:bold;margin-bottom:60px;margin-top:60px;">      <?php echo $web['home_title']; ?> <span style="color:#3498db;">is your local shopping centre.</span><br>
	  <span class="ui huge normal header inverted" style="font-weight: 300;font-size: 20px;">
			Find products and stors in your local area or around the world from small bussiness.
	  </span>
	  </h1>
    </div>
  </div>
...
<?php if($web['currency'] == NULL && $web['paypal_email'] == NULL) { ?><div role="alert" class="alert color orange-color">You must edit your payment settings in order to use your store.</div><?php } else { ?>
  ...
  <div class="ui icon input">
    <select name="phps_brand_query" style="border-top-right-radius:0px;border-bottom-right-radius:0px;" if(isset($_POST['phps_brand_query'])) { echo 'value="'.mysql_real_escape_string($_POST['phps_brand_query']).'"'; }>
...
while($row_brand = mysql_fetch_array($query))
{
  echo "<option value=".$row_brand['id'].">".$row_brand['name']."</option>";
											...
											<select name="phps_size_query">
											   <option value="">Select All</option>
											    <?php
											   $query_size = mysql_query("SELECT * FROM purchasify_sizes ORDER BY id ASC");
while($row_size = mysql_fetch_array($query_size))
{
  echo "<option value=".$row_size['id'].">".$row_size['name']."</option>";
    ...
    <input style="border-top-right-radius:0px;border-bottom-right-radius:0px;" type="text" name="phps_query" placeholder="Search Bourke" <?php if(isset($_POST['phps_query'])) { echo 'value="'.mysql_real_escape_string($_POST['phps_query']).'"'; } ?>><input style="border-top-right-radius:0px;border-bottom-right-radius:0px;" type="text" name="phps_query1" placeholder="Search Bourke" <?php if(isset($_POST['phps_query1'])) { echo 'value="'.mysql_real_escape_string($_POST['phps_query1']).'"'; } ?>>
  ...
  <div class="ui items">
	<h1 class="ui header thin">Items from the community</h1>
	<?php
		$page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
		$limit = 50000;
		$startpoint = (int)(($page * $limit) - $limit);
		$statement = "`purchasify_items`";
		$sql = mysql_query("SELECT * FROM {$statement} WHERE membership='' ORDER BY RAND() LIMIT {$startpoint} , {$limit}");
		if(mysql_num_rows($sql)>0) {
		while($row = mysql_fetch_array($sql)) {
	?>
  <!-- PRODUCT START -->
  ...
  <?php } ?>
	...
	<!-- FOOTER CODE -->
	<?php include($_SERVER['DOCUMENT_ROOT']."/hautehire/static/gen/modal/footer-modal.php"); ?>
    ...
    <script type="IN/Share" data-url="https://developer.linkedin.com/plugins/share" data-counter="top"></script>
    <!-- linkedin -->
</body>
</html>