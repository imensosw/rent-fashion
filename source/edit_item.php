...
if(isset($_POST['phps_update_info'])) 
{
    ...
    $category_id = 0;
	if(empty($name) or empty($price_extended) or empty($description)) 
	{ 
	   echo '<div class="alert color red-color"><p align="center">Required fiels: Product name, description and price.</p></div>'; 
	}
	else 
	{
	   $update = mysql_query("UPDATE purchasify_items SET name='$name',description='$description',price_regular='$price_regular',price_extended='$price_extended',demo_url='$demo_url',screenshots_url='$screenshots_url',brand='$brand',size='$size',country='$country',state='$state',cities='$city',address='$address',featured='$featured',membership='$membership',video_url='$video_url',category_id='$category_id' WHERE id='" . mysql_real_escape_string($row['id']) . "'") or die(mysql_error());
	   
		$update = mysql_query("UPDATE purchasify_purchases SET name='$name' WHERE item_id='" . mysql_real_escape_string($row['id']) . "'");
		$row = mysql_fetch_array(mysql_query("SELECT * FROM purchasify_items WHERE id='" . mysql_real_escape_string($row['id']) . "'"));
...
if(isset($_POST['phps_update_tpfiles'])) 
{
	...
	{ 
		echo '<div class="alert color red-color"><p align="center">Please select images.</p></div>'; 
	}
	elseif(!empty($thumbnail) && empty($preview)) 
	{
		@unlink($row['thumbnail']);
		if(@move_uploaded_file($_FILES['thumbnail_file']['tmp_name'], $thumbnail_path)) 
		{ 
			$error = 0; 
		} 
		else 
		...
		if($error == 0) 
		{
			$update = mysql_query("UPDATE purchasify_items SET thumbnail='$thumbnail_path' WHERE id='$id'");
			echo '<div class="alert color blue">';
	...
	{
		@unlink($row['preview']);
		if(@move_uploaded_file($_FILES['preview_file']['tmp_name'], $preview_path)) 
		{ 
		  $error = 0; 
		} 
		else 
		...
		if($error == 0) 
		{
			$update = mysql_query("UPDATE purchasify_items SET preview='$preview_path' WHERE id='$id'");
			$update = mysql_query("UPDATE purchasify_purchases SET preview='$preview_path' WHERE item_id='" . mysql_real_escape_string($row['id']) . "'");
			echo '<div class="alert color blue">';
	...
	elseif(!empty($preview) && !empty($thumbnail)) 
	{
		...
		if($error == 0) 
		{
			$update = mysql_query("UPDATE purchasify_items SET thumbnail='$thumbnail_path',preview='$preview_path' WHERE id='$id'");
			$update = mysql_query("UPDATE purchasify_purchases SET preview='$preview_path' WHERE item_id='" . mysql_real_escape_string($row['id']) . "'");
			echo '<div class="alert color blue">';
...
if(isset($_POST['phps_update_mfile'])) 
{
	...
	if(empty($main)) 
	{ 
		echo '<div class="alert color red-color"><p align="center">Please select main file.</p></div>'; 
	}
	else 
	{
		@unlink($row['main_file']);
		if(@move_uploaded_file($_FILES['main_file']['tmp_name'], $main_path)) 
		{ 
			$error = 0; 
		} 
		else 
		...
		if($error == 0) 
		{
			$update = mysql_query("UPDATE purchasify_items SET main_file='$main_path' WHERE id='$id'");
			$get_buyers = mysql_query("SELECT * FROM purchasify_purchases WHERE item_id='$id'");
    ...
    <div class="ui grid full-width">
      <div class="equal height two column row">
        ...
        <div class="right aligned column">
          ...
          <a class="ui tiny blue button product-share" href="<?php echo $web['url']; ?>delete/<?php echo protect($row['id']); ?>">
            Delete product
          </a>
          
       <a class="ui tiny basic button" href="<?php echo $web['url']; ?>preview/<?php echo protect($row['id']); ?>" target="_blank">
            View product
          </a>
        </div>
								...
								<?php } ?>
									...
									<div class="field">
									  <div class="ui input join">
                                        ...
                                        <input class="join" type="text" placeholder="1.00" value="<?php echo protect($row['price_regular']); ?>" name="price_regular" id="price_extended" required>
                                      </div>
									</div>
									<div class="field">
									  <div class="ui input join">
...
while($row_brand = mysql_fetch_array($query))
{
  if(protect($row['size']) == $row_brand['id'])
  {
    echo "<option value=".$row_brand['id']." selected>".$row_brand['name']."</option>";
  }
  else
  {
    ...
    <select name="country" id="country">
        ...
        if($rowCount > 0)
        {
            ...
            {
                if(protect($row['country']) == $row_country['id'])
                {
        ...
        ?>
					...
					if(isset($row['state']) && !empty($row['state']))
				        {
				           $query = mysql_query("SELECT * FROM purchasify_states where country_id = " . $row['country'] . " ORDER BY name ASC");
                                           //Count total number of rows
                                           $rowCount = mysql_num_rows($query) or die(mysql_error());
                                           ?>
					        ...
					        if($rowCount > 0)
					        {
					            ...
					            {
					                if(protect($row['state']) == $row_state['id'])
					                {
				        ...
				        ?>
									...
									<input type="hidden" name="city_selected" id="city_selected" value="<?php echo protect($row['cities']); ?>" />
                                                                        <select name="city[]" id="city" class="selectpicker" multiple="multiple">
									    ...
									    <div class="equal height two column row fields product-upload-price-field">
											...
											<input type="text" name="address" placeholder="Enter Address" value="<?php echo protect($row['address']); ?>"/>
										</div>
									  </div>
									</div>-->
									<div class="ui grid">
  ...
  else
  {
            ...
            <div class="ui three column grid">
                <div class="row equal height">
                    ...
                    <div class="middle aligned three wide column">
                        ...
                        <a id="delete-product" class="ui basic button" href="<?php echo $web['url']; ?>delete/<?php echo protect($row['id']); ?>">
                            Delete product
                        </a>
                    </div>
	...
	<div class="centered footer">
	  ...
	  <p class="footer-copyright">
		<span class="item">
		  &copy; <?php date_default_timezone_set('Asia/Kolkata'); echo date("Y"); ?> <?php echo $web['sitename']; ?>, All rights reserved | Made with <i style="color:#ea6052;" class="fa fa-heart"></i> by <strong>The Spreadrr Team</strong>
		</span>
	  </p>
	</div>
	<!-- FOOTER END -->
    ...
    if(stateID>0 && city_selected!="")
    {
       ...
       if(stateID)
		...
		//$('#city_ul').find('.dropdown-menu').remove();
         ...
         xhttp.open("GET", "http://dev.imenso.co/hautehire/source/ajaxData.php?state_id="+stateID+" & city_selected="+city_selected, true);
         xhttp.send();
        }
        else
        {
