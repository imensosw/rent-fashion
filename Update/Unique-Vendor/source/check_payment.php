...
<?php
  ...
  $sql = mysql_query("SELECT * FROM purchasify_items WHERE custom_url='" . mysql_real_escape_string($_POST['item_number']) . "'");
					...
					if ($payment_status == 'Completed') {
							...
							if ($web['paypal_email'] == $web['paypal_email']) {
								...
								if ($payment_amount == $pricee && $web['currency'] == $web['currency']) {
									...
									$update = mysql_query("UPDATE purchasify_settings SET earnings=$earnings+$payment_amount");
									$random_password=md5(uniqid(rand()));
									$email_code=substr($random_password, 0, 12);
									$time = time();
									$insert = mysql_query("INSERT purchasify_purchases (item_id,usern,buyer_email,time,code,name,price,membership) VALUES ('$item_number','" . mysql_real_escape_string($_SESSION['ps_usern']) . "','$user_email','$time','$email_code','$item_name','$payment_amount','" . mysql_real_escape_string($row['membership']) . "')");
									$to = $user_email;
			...
			<table style=\"border:none;padding:0 18px;margin:50px auto;width:500px;\">
				<tbody>
					...
					<tr width=\"100%\">
						...
						<h1 style=\"font-size:20px;margin:0;color:#333;\">Hey " . protect($_SESSION['ps_usern']) . ",</h1>
