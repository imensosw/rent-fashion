...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="<?php echo $web['url']; ?>static/bar/bugme.css">
...
<body class="landing-page">
  ...
  <div class="sellfy-landing-header">
    <div class="ui grid global-wrap">
	  ...
	  <?php if(!$_SESSION['ps_usern']) { ?>
		  ...
		  <a class="ui tiny flat green button hide-mobile" href="<?php echo $web['url']; ?>account/register">Create an account</a>
	  <?php } ?>
  ...
  <div class="landing-header">
    <div class="ui global-wrap">
	  <h1 class="ui huge normal header inverted" style="text-transform:uppercase;font-weight:bold;margin-bottom:60px;margin-top:60px;">      <?php echo $web['home_title']; ?> <span style="color:#3498db;">MARKET</span><br>
...
<div class="centered statistics">
  <div class="ui tiny statistic">
    <div class="value">
      <?php $downloads = 0; $sql = mysql_query("SELECT * FROM purchasify_items ORDER BY id"); if(mysql_num_rows($sql)>0) { while($row = mysql_fetch_array($sql)) { $downloads = $downloads+$row['downloads']; } } echo number_format($downloads); ?>
    </div>
    <div class="label">Downloads</div>
  </div>
  <div class="ui tiny statistic">
    <div class="value">
      <?php $sales = 0; $sql = mysql_query("SELECT * FROM purchasify_items ORDER BY id"); if(mysql_num_rows($sql)>0) { while($row = mysql_fetch_array($sql)) { $sales = $sales+$row['sales']; } } echo number_format($sales); ?>
    </div>
    <div class="label">Sales</div>
  </div>
  <div class="ui tiny statistic">
    <div class="value">
      <?php if($total_items == 0) { ?>0<?php } ?><?php if($total_items > 0) { ?><?php echo $total_items; ?><?php } ?>
    </div>
...
<?php if($web['currency'] == NULL && $web['paypal_email'] == NULL) { ?><div role="alert" class="alert color orange-color">You must edit your payment settings in order to use your store.</div><?php } else { ?>
  ...
  <div class="ui icon input">
    <input style="border-top-right-radius:0px;border-bottom-right-radius:0px;" type="text" name="phps_query" placeholder="Search items..." <?php if(isset($_POST['phps_query'])) { echo 'value="'.mysql_real_escape_string($_POST['phps_query']).'"'; } ?>>
  ...
  <div class="ui items">
	<?php
		...
		$sql = mysql_query("SELECT * FROM {$statement} WHERE featured='on' ORDER BY id DESC LIMIT {$startpoint} , {$limit}");
  ...
  <div class="ui segment item product-item -view " style="margin: 8px!important;" itemscope itemtype="http://schema.org/Product">
      <div class="ui image product-image">
		...
		<a class="product-image-static lazy-load" title="<?php echo protect($row['name']); ?>" target="_top" href="<?php echo $web['url']; ?>p/<?php echo protect($row['custom_url']); ?>" style="display: block;height:<?php echo $web['home_dimensions']; ?>px; background-image: url(<?php echo $web['url']; echo $row['thumbnail']; ?>);"></a>
	  </div>
		...
		<div class="ui grid">
		  <div class="row product-meta-info">
			<div class="twelve wide column">
			  <h3 class="ui header">
				<a class="truncate" title="<?php echo $row['name']; ?>" target="_top" href="<?php echo protect($web['url']); ?>p/<?php echo $row['custom_url']; ?>">
  ...
  <!-- PRODUCT END -->
	...
	<?php
			...
			echo info('Currently there are no featured products.');
		}
	?>
  ...
  <div class="ui items">
	<h1 class="ui header thin">All items available</h1>
	<?php
		$page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
		$limit = 50000;
  ...
  <div class="ui segment item product-item -view " style="margin: 8px!important;" itemscope itemtype="http://schema.org/Product">
      <div class="ui image product-image">
		...
		<a class="product-image-static lazy-load" title="<?php echo $row['name']; ?>" target="_top" href="<?php echo $web['url']; ?>p/<?php echo $row['custom_url']; ?>" style="display: block;height:<?php echo $web['home_dimensions']; ?>px; background-image: url(<?php echo $web['url']; echo $row['thumbnail']; ?>);"></a>
	  </div>
	  <div class="row product-meta">
		<div class="ui grid">
		  <div class="row product-meta-info">
  ...
  <?php } ?>
	...
	<div class="centered landing-cta-bottom">
		...
		<?php if(idinfo($_SESSION['ps_usern'],"role") == 0) { ?>
		  <?php if($_SESSION['ps_usern']) { ?>
		  <a class="ui button green big" href="<?php echo $web['url']; ?>account/profile">Access to my account <i class="fa fa-arrow-right"></i></a>
		  <?php } ?>
		<?php } ?>
	</div>
  </div>
