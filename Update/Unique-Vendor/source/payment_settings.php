...
<html lang="en">
	<head>
		<meta charset="utf-8">
		<title>Payment Settings - <?php echo $web['title']; ?></title>
...
<body>
	...
	<?php
			if(isset($_POST['phps_save'])) {
				...
				if(empty($currency) or empty($paypal_email)) { echo '<div class="alert color red-color"><p align="center">Please choose currency and PayPal email address.</p></div>'; }
				elseif(!empty($paypal_email) && !isValidEmail($paypal_email)) { echo error("Please enter valid PayPal email address."); }
				else {
					$update = mysql_query("UPDATE purchasify_settings SET currency='$currency',paypal_email='$paypal_email'");
		...
		<div class="ui fluid vertical menu">
		  ...
		  <div class="item submenu-wrap">
			<div class="menu submenu">
				...
				<a class="green item active" href="<?php echo $web['url']; ?>admin/payments" data-pushstate="true">Payment</a>
			</div>
		  </div>
  ...
  <div class="ui grid">
    ...
    <div class="equal height one column row">
      <div class="column">
        ...
        <p>Add/edit your payment options.</p>
  ...
  <div class="userpage-content">
    <div class="ui middle aligned grid billing-form">
        <div class="row">
          <div class="column field primary-currency">
            <h3 class="ui header normal">Primary account currency</h3>
              <select name="currency" class="ui fluid selection dropdown">
				<option value=""></option>
				<option value="USD" <?php if($web['currency'] == "USD") { echo 'selected'; } ?>>USD</option>
				<option value="EUR" <?php if($web['currency'] == "EUR") { echo 'selected'; } ?>>EUR</option>
				<option value="GBP" <?php if($web['currency'] == "GBP") { echo 'selected'; } ?>>GBP</option>
			  </select>
          </div>
        </div>
        ...
        <div class="row two column paypal-settings">
          <div class="column">
            <img class="ui image" src="https://img.spreadrr.com/i/896331326paypal-logo.png" alt="PayPal">
          </div>
        ...
        <div class="row">
          <div class="column">
            <div class="ui ignored warning message visible">
                Please make sure your PayPal account has the same currency ballances you are using for your store.
            </div>
            <input type="email" value="<?php echo $web['paypal_email']; ?>" id="paypal_email" name="paypal_email">
          </div>
        </div>
    </div>
  </div>
