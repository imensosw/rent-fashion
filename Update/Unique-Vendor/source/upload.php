...
<body>
	...
	<?php
			if(isset($_POST['phps_upload'])) {
						...
						$price_regular = mysql_real_escape_string(protect($_POST['price_regular']));
						$price_extended = mysql_real_escape_string(protect($_POST['price_extended']));
						...
						$demo_url = mysql_real_escape_string(protect($_POST['demo_url']));
						$screenshots_url = mysql_real_escape_string(protect($_POST['screenshots_url']));
						$featured = mysql_real_escape_string(protect($_POST['featured']));
						$membership = mysql_real_escape_string(protect($_POST['membership']));
						$video_url = mysql_real_escape_string(protect($_POST['video_url']));
						$custom_url = mysql_real_escape_string(protect($_POST['custom_url']));
						$category_id = mysql_real_escape_string(protect($_POST['category_id']));
						...
						elseif($urlchecker > 1) { echo error("Error! Custom URL already exists."); }
							...
							if($error == 0) {
								$update = mysql_query("UPDATE purchasify_items SET thumbnail='$thumbnail_path',preview='$preview_path',main_file='$main_path' WHERE id='$row[id]'");
								$link = '<a href="'.$web[url].'item/'.$row['custom_url'].'">'.$row['name'].'</a>';
								echo '<div class="alert color blue">';
								echo '<p align="center"><a href="'.$web['url'].'item/'.$row['custom_url'].'"><span style="color:white">Item was added successfully. Preview here: '.$row['name'].'</span></a></p>';
								echo '</div>';
							} else {
	  ...
	  <?php } ?>
	   <div class="column main-content" id="content_container">
		...
		<?php if($web['paypal_email'] == NULL) { ?>
		  <div class="ui ignored warning message visible">
			You need to edit your Payment Options before adding a product. <a style="color:#333!important;font-weight:bold;text-decoration:underline;" href="<?php echo $web['url']; ?>admin/payments">Let's go!</a>
		  </div>
		<?php } else { ?>
